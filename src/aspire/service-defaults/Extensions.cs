...
namespace imediatus.Aspire.ServiceDefaults;
...
public static class Extensions
{
    ...
    public static IHostApplicationBuilder ConfigureOpenTelemetry(this IHostApplicationBuilder builder)
    {
        ...
        var versionAttribute = entryAssembly?.GetCustomAttributes(false)
            ...
            .FirstOrDefault();
        var resourceServiceName = entryAssemblyName?.Name ?? "DefaultServiceName";
        var resourceServiceVersion = versionAttribute?.InformationalVersion ?? entryAssemblyName?.Version?.ToString();
        var attributes = new Dictionary<string, object>
        {
            ["host.name"] = Environment.MachineName,
            ["service.names"] =
                "imediatus.WebApi.Host", //builder.Configuration["OpenTelemetrySettings:ServiceName"]!, //It's a WA Fix because the service.name tag is not completed automatically by Resource.Builder()...AddService(serviceName) https://github.com/open-telemetry/opentelemetry-dotnet/issues/2027
        ...
        builder.Services.AddOpenTelemetry()
            .WithMetrics(metrics =>
            {
                metrics.SetResourceBuilder(resourceBuilder)
                    ...
                    .AddMeter(MetricsConstants.Workspace)
                    .AddMeter(MetricsConstants.Orchestrator)
                    .AddMeter(MetricsConstants.Connection);
            })
            .WithTracing(tracing =>
            {
