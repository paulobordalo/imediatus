...
using imediatus.Framework.Infrastructure.Tenant.Abstractions;
using imediatus.Shared.Authorization;

namespace imediatus.Framework.Infrastructure.Tenant;
public sealed class ImediatusTenantInfo : IImediatusTenantInfo
{
    public ImediatusTenantInfo()
    {
    }

    public ImediatusTenantInfo(string id, string name, string? connectionString, string adminEmail, string? issuer = null)
    {
        Id = id;
        Identifier = id;
        Name = name;
    ...
    public void SetValidity(in DateTime validTill) =>
        ValidUpto = ValidUpto < validTill
            ? validTill
            : throw new ImediatusException("Subscription cannot be backdated.");
    ...
    string? IImediatusTenantInfo.ConnectionString { get => ConnectionString; set => ConnectionString = value ?? throw new InvalidOperationException("ConnectionString can't be null."); }
}
