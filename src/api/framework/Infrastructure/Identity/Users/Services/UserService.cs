...
using imediatus.Framework.Core.Identity.Users.Features.AssignUserRole;
using imediatus.Framework.Core.Identity.Users.Features.RegisterUser;
using imediatus.Framework.Core.Identity.Users.Features.ToggleUserStatus;
using imediatus.Framework.Core.Identity.Users.Features.UpdateUser;
using imediatus.Framework.Core.Jobs;
...
internal sealed partial class UserService(
    ...
    SignInManager<ImediatusUser> signInManager,
    ...
    ) : IUserService
{
    ...
    public async Task<string> ConfirmEmailAsync(string userId, string code, string tenant, CancellationToken cancellationToken)
    {
        ...
        _ = user ?? throw new ImediatusException("An error occurred while confirming E-Mail.");
        ...
        return result.Succeeded
            ...
            : throw new ImediatusException(string.Format("An error occurred while confirming {0}", user.Email));
    }
    ...
    {
        EnsureValidTenant();
        return await userManager.FindByEmailAsync(email.Normalize()) is ImediatusUser user && user.Id != exceptId;
    }
    ...
    {
        EnsureValidTenant();
        return await userManager.Users.FirstOrDefaultAsync(x => x.PhoneNumber == phoneNumber) is ImediatusUser user && user.Id != exceptId;
    }
    ...
    public async Task<RegisterUserResponse> RegisterAsync(RegisterUserCommand request, string origin, CancellationToken cancellationToken)
    {
        ...
        if (!result.Succeeded)
        {
            ...
            throw new ImediatusException("error while registering a new user", errors);
        }

        // add basic role
        await userManager.AddToRoleAsync(user, ImediatusRoles.Basic);
    ...
    public async Task ToggleStatusAsync(ToggleUserStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        bool isAdmin = await userManager.IsInRoleAsync(user, ImediatusRoles.Admin);
        if (isAdmin)
        {
            throw new ImediatusException("Administrators Profile's Status cannot be toggled");
        }
    ...
    public async Task UpdateAsync(UpdateUserCommand request, string userId)
    {
        ...
        {
            user.ImageUrl = await storageService.UploadAsync<ImediatusUser>(request.Image, FileType.Image);
        ...
        if (!result.Succeeded)
        {
            throw new ImediatusException("Update profile failed");
        }
    }

    public async Task DeleteAsync(string userId)
    {
        ImediatusUser? user = await userManager.FindByIdAsync(userId);
        ...
        if (!result.Succeeded)
        {
            ...
            throw new ImediatusException("Delete profile failed", errors);
        }
    }

    private async Task<string> GetEmailVerificationUriAsync(ImediatusUser user, string origin)
    {
        EnsureValidTenant();
    ...
    public async Task<string> AssignRolesAsync(string userId, AssignUserRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await userManager.IsInRoleAsync(user, ImediatusRoles.Admin)
            && request.UserRoles.Exists(a => !a.Enabled && a.RoleName == ImediatusRoles.Admin))
        {
            // Get count of users in Admin Role
            int adminCount = (await userManager.GetUsersInRoleAsync(ImediatusRoles.Admin)).Count;
            ...
            if (user.Email == TenantConstants.Root.EmailAddress)
            {
                ...
                {
                    throw new ImediatusException("action not permitted");
                }
            }
            else if (adminCount <= 2)
            {
                throw new ImediatusException("tenant should have at least 2 admins.");
            }
        }
