...
using imediatus.Framework.Core.Auth.Jwt;
...
namespace imediatus.Framework.Infrastructure.Identity.Tokens;
public sealed class TokenService : ITokenService
{
    private readonly UserManager<ImediatusUser> _userManager;
    private readonly IMultiTenantContextAccessor<ImediatusTenantInfo>? _multiTenantContextAccessor;
    private readonly JwtOptions _jwtOptions;
    private readonly IPublisher _publisher;
    public TokenService(IOptions<JwtOptions> jwtOptions, UserManager<ImediatusUser> userManager, IMultiTenantContextAccessor<ImediatusTenantInfo>? multiTenantContextAccessor, IPublisher publisher)
    {
    ...
    private async Task<TokenResponse> GenerateTokensAndUpdateUser(ImediatusUser user, string ipAddress)
    {
    ...
    private string GenerateJwt(ImediatusUser user, string ipAddress) =>
    ...
    private List<Claim> GetClaims(ImediatusUser user, string ipAddress) =>
        new List<Claim>
        {
            ...
            new(ImediatusClaims.Fullname, $"{user.FirstName} {user.LastName}"),
            new(ClaimTypes.Surname, user.LastName ?? string.Empty),
            new(ImediatusClaims.IpAddress, ipAddress),
            new(ImediatusClaims.Tenant, _multiTenantContextAccessor!.MultiTenantContext.TenantInfo!.Id),
            new(ImediatusClaims.ImageUrl, user.ImageUrl == null ? string.Empty : user.ImageUrl.ToString())
        };
